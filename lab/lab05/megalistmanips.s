...
mapLoop:
    # correct3: we need the address og array, not linked list
    lw t1, 0(s0)      # load the address of the array of current node into t1
    lw t2, 4(s0)        # load the size of the node's array into t2
    # correct5: count by bytes, each int has four bytes
    slli t3, t0, 2
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0

    # correct4: when we call function, we need to save t register
    addi sp sp -12
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    ...
    # correct2: address of function is in s1
